...
for i = 1:n_tri
	...
	if any(i == ind_left_edge) || any(i == ind_right_edge)
		...
		if ~isempty(ind_in_p)
			...
			% The indices in p where these minimum values are located. (The
			% points on the opposite edge which are closest in height to
			% the selected ones on the current edge).
