...
r_index = zeros(length(wavelength),4);
...
%             end
            ...
            trans(counter2) = ((1-r12_cal^2)*exp(1i*k0*d*n_cal))/(1 - r12_cal^2*exp(2i*k0*d*n_cal));
            accuracy = abs(trans - t)/abs(t);
            trans;
            t;
            real_n(1,counter2) = n_cal;
            real_n(2,counter2) = abs(real(t - trans(counter2)))/abs(real(t)) + abs(imag(t - trans(counter2)))/abs(imag(t));
            [M,I] = min(real_n(2,:));
            r_index(h,1) = real_n(1,I);
            r_index(h,2) = real_n(2,I);
            r_index(h,3) = trans(I);
            r_index(h,4) = t;
            end
